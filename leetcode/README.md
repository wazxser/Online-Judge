...
要求时间复杂度O(n)，就以空间换时间了。设置一个集合unordered_set存储数组所有元素，然后遍历数组，遍历到一个元素时，往前往后查找连续的数字，通过set查找，如果存在，则erase，不存在结束。
...
从尾部开始遍历，遇到a[i-1] < a[i]停止，这样a[i]到数组最后一个元素是降序的排列的，再从末尾开始，找到第一个比a[i-1]大的元素，交换该元素和a[i-1]，再将a[i-1]之后的元素reverse，得到结果。


#### 剑指 数组中出现超过一半的数字   169. 多数元素
数组中有一个数字出现的次数超过数组长度的一半，请找出这个数字。例如输入一个长度为9的数组{1,2,3,2,2,2,5,4,2}。由于数字2在数组中出现了5次，超过数组长度的一半，因此输出2。如果不存在则输出0。



## 链表
#### 剑指 两个链表的第一个公共结点   160. 相交链表
输入两个链表，找出它们的第一个公共结点。
...
用快慢指针找到链表的中点，分开为两个链表，然后将两个链表合并为一个有序链表。

#### 234. 回文链表 
判断一个链表是否是回文链表
快慢指针找到链表的中点，用一个栈压入前一段链表，再和后一段链表比较。
...
#### 剑指 二叉树的深度 104. 二叉树的最大深度
从根节点到叶节点最深的一条路径

1、层序遍历计算
2、递归计算，取左右子树的最大深度+1返回
...
检查根节点是否为p、q之一，如果是返回，如果不是，递归左子树，递归右子树，如果左右递归均不为空，返回根节点，如果一个空，返回另一个不空的结点。
...
遍历矩阵，如果为1，增加岛屿数量，上下左右搜索，搜索到的位置置0
...
说明：数值范围为 [−231,  231 − 1]。如果数值超过这个范围，请返回  INT_MAX (231 − 1) 或 INT_MIN (−231) 。
...
#### 剑指 包含min函数的栈  155.最小栈
定义栈的数据结构，请在该类型中实现一个能够得到栈中所含最小元素的min函数（时间复杂度应为O（1））。

两个栈，一个栈保持数据正常的压入弹出顺序，另一个栈存储最小的元素。
